...
**Private to its user.** The daemon, before it takes the lock, and `beam daemon
--detach`, before it opens `daemon.log`, check what they are about to trust:
...
socket in `/tmp` is refused, not `chmod`ed); a socket that needs a short path goes in a
private directory such as `$XDG_RUNTIME_DIR`.
