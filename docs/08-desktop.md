...
The desktop passes `BEAM_CONFIG_DIR` explicitly, as an absolute path, to the daemon it
spawns and every process it starts, so that they all reach the same daemon.
