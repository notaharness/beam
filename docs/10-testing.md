...
| private to its user | a `$BEAM_DIR`, `run/` or socket directory others can write, or a key, fleet, state or log file others can read: the daemon and `--detach` exit 1 naming the path and the `chmod`, the mode unchanged; a command whose spawned daemon refuses fails at once with that reason |
