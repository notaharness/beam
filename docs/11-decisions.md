...
| D34 | A serialized envelope is at most 960 KiB, a `msg.defer` reason 1 KiB, and `msg.queue` pages stop short of 1 MiB. | Every envelope then fits one 1 MiB control line with the `mail` event or `msg.queue` reply around it. | 05, 06 |
