...
| D16 | Daemon outlives clients; only explicit stop stops it; one connect-or-spawn helper. The one exception is a daemon the desktop spawned (D53). | A listener that dies with the UI is not a listener. | 06 |
