...
| D45 | Every ceremony returns its result through a worker slot, sealed to a per-ceremony key in the URL's fragment; the loopback listener, its `/cb` page and the `ssh -L` instruction are gone. The CLI prints the URL, draws it as a QR code and opens the browser when there is one. | One path for desktop, headless and phone. The exposure to a URL the owner did not start comes from the page being able to write a slot at all; a loopback path kept for machines with a browser would not remove it. Loopback's presence proof is given up, and with it protection from an onlooker at first enrolment on a desktop, where the URL is not drawn and the owner's page reports a lost race (01, The relayed result). | 01, 02, 07 |
...
| D50 | A slot result that does not open, or does not parse, ends the ceremony `ceremony-state`. | It is not an answer to this ceremony: junk from the worker, or a write sealed to another key. | 02, 06 |
| D51 | The QR code is drawn only when stdout is a terminal, at level L, in half blocks with explicit SGR black on white and a four-module quiet zone; `github.com/boombuler/barcode` encodes it. | Level L keeps a typical URL under 80 columns; explicit colours read on light and dark themes alike; a pipe gets the URL alone. The encoder is small, current and has no dependencies. | 07, 09 |
