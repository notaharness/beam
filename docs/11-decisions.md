...
| D56 | A stopping daemon sends subscribers `shutdown { reason }` before closing, and emits each event to every subscriber at once. | Without it a client cannot tell a deliberate stop from a crash and respawns a daemon someone just stopped. Sent one subscriber after another, a stalled one would delay the rest by its full timeout. | 06, 08 |
