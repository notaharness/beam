...
| D34 | A serialized envelope is at most 960 KiB, a `msg.defer` reason 1 KiB, and `msg.queue` pages stop short of 1 MiB. JSON lines are written without HTML escaping, so an envelope never goes out larger than stored. | Every envelope then fits one 1 MiB control line with the `mail` event or `msg.queue` reply around it; escaping would swell `<` sixfold past it. | 05, 06 |
