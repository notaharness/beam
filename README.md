...
The commands open a browser when the machine has one, and always print the ceremony URL
with a QR code under it. On a headless machine, scan the code with your phone and approve
there; the result comes back through `beam.n10.is`, sealed to a key only the daemon
holds. Anyone who scans the code can answer it too, so show it only where you alone can
see it, and approve only a page that names the machine you are enrolling.
